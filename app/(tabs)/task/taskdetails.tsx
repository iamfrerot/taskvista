  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://167.99.192.166:4000/api/v1/tasks/edit/${task._id}`,
        {
          method: "PATCH",
...
const TaskInfo = ({
  ...
  useEffect(() => {
    async function fetchDevsProfiles(userIds: string[]) {
      const userProfiles = [];
      const baseUrl = "http://167.99.192.166:4000/api/v1/users/view/";
  ...
  return (
    <View className={`p-4 rounded-lg ${getStatusBgColor(task?.priority)} `}>
      <Text className="text-2xl font-bold mb-4 text-white-100">
        {task?.name}
      </Text>
      <View className={"flex-row justify-between"}>
        <View className={"flex-row items-center"}>
          ...
          <View className={"ml-3"}>
            <Text className={"text-sm text-gray-50 font-oregular"}>
              Due Date
            </Text>
        ...
        <View className="flex-row">
          ...
          <View className="ml-3">
            <Text className="text-sm text-white-100 font-osemibold">
              Developers
            </Text>
            {developersProfiles ? (
              <AvatarGroup
